...
export abstract class Document<
    DataType extends IDocument,
    DBSchema extends IDocumentDataSchema,
    WFSchema extends IDocumentWFSchema,
> {
        ...
        switch (arg.length) {
            ...
            default:
                throw new DocumentError(
                    ...
                    `Couldn't create class '${this.constructor.name}' too much data`
                );
        }
    }
    ...
    get dataSchema(): DBSchema {
        throw new DocumentError(
            DocumentErrorCode.unknown,
            `Abstract class '${this.constructor.name}' has no data schema. Implement getter of 'dataSchema' property`
        );
    }
    ...
    get wfSchema(): WFSchema {
        throw new DocumentError(
            DocumentErrorCode.unknown,
            `Abstract class '${this.constructor.name}' has no workflow schema. Implement getter of 'wfSchema' property`
        );
    }
    ...
    get sqlConnection(): Connection {
        if (Document._sqlConnection === undefined)
            throw new DocumentError(
                DocumentErrorCode.sql_connection_error,
    ...
    static async createSQLConnection(): Promise<Connection> {
        if (Document._sqlConnection === undefined) {
            ...
            mconsole.sqlinfo(
                `Creating database connection: database: '${db_name}'; user: '${db_user}'; pwd: ${db_pwd ? "'******'" : '-'}; port: '${db_port}'`
            );
    ...
    get data(): DataType {
        if (this._data === undefined)
            throw new DocumentError(
                DocumentErrorCode.abstract_method,
                `Object typeof '${this.constructor.name}' id = '${this._id}' has no data but used for active manipulation`
            );
        return this._data;
    }
    ...
    async save(): Promise<DataType> {
        ...
        if (this._id === undefined) {
            schemaDefinedFields = this.dataSchema.fields.filter(
                (field) => (this.data as any)[field.name] !== undefined
            );
            wfSchemaDefinedFields = DocumentBaseSchema.filter(
                (field) => (this.data as any)[field.name] !== undefined
            );
        }
        let sql: string;
        ...
        const params = [
            ...schemaDefinedFields.map(
                (field) => (this.data as any)[field.name]
            ),
            ...wfSchemaDefinedFields.map(
                (field) => (this.data as any)[field.name]
            ),
        ];
        ...
        for (const [propName, propValue] of Object.entries(this.data)) {
            if (
                -1 ===
                    this.dataSchema.fields.findIndex(
                        (field) => field.name === propName
                    ) &&
                -1 ===
                    this.dataSchema.related?.findIndex(
                        (relObj) => relObj.tableName === propName
                    ) &&
                -1 ===
                    DocumentBaseSchema.findIndex(
                        (field) => field.name === propName
                    )
            )
                console.warn(
                    `Property '${propName}' is absent in schema of '${this.dataSchema.tableName}'`
                );
        }
        ...
        while (true) {
            try {
                const [sh, fields] =
                    await this.sqlConnection.query<ResultSetHeader>(
                        sql,
                        params
                    );
            ...
            for (const relObj of this.dataSchema.related) {
                if (relObj.tableName in this.data) {
                    ...
                    for (const element of arrProp) {
                        ...
                        const initialState = this.wfSchema.related
                            ?.filter((el) => el.tableName === relObj.tableName)
                            ?.at(0)?.initialState;
                        ...
                        if (element[relObj.idFieldName] === undefined) {
                            schemaDefinedFields = relObj.fields.filter(
                                (field) =>
                                    (element as any)[field.name] !== undefined
                            );
                            wfSchemaDefinedFields = DocumentBaseSchema.filter(
                                (field) =>
                                    (element as any)[field.name] !== undefined
                        ...
                        const params = [
                            this._id,
                            ...schemaDefinedFields.map(
                                (field) => (element as any)[field.name]
                            ),
                            ...wfSchemaDefinedFields.map(
                                (field) => (element as any)[field.name]
                            ),
                        ...
                        if (element[relObj.idFieldName] !== undefined)
                            params.push(element[relObj.idFieldName]);
                        mconsole.sqlq(sql, params);
                        while (true) {
                            try {
                                await this.sqlConnection.query<ResultSetHeader>(
    ...
    protected async createMainTable() {
        mconsole.sqlinfo(
            `Creating main table of schema '${this.dataSchema.tableName}'`
        );
        ...
        if (this.dataSchema.indexes !== undefined)
            for (const key of this.dataSchema.indexes) {
                ...
                await this.sqlConnection.query(sql);
                mconsole.sqlinfo(
                    `Index of '${this.dataSchema.tableName}' has created successfully`
                );
            }
        mconsole.sqlinfo(
            `Main table of schema '${this.dataSchema.tableName}' has created successfully`
        );
    }

    protected async createRelatedTable(tableSchema: IDocumentDataSchema) {
        mconsole.sqlinfo(
            `Creating related table '${tableSchema.tableName}' of schema '${this.dataSchema.tableName}'`
        );
        ...
        await this.sqlConnection.query(sql);
        mconsole.sqlinfo(
            `Related table '${tableSchema.tableName}' of schema '${this.dataSchema.tableName}' has created successfully`
        );
        ...
        if (tableSchema.indexes !== undefined)
            for (const key of tableSchema.indexes) {
                ...
                await this.sqlConnection.query(sql);
                mconsole.sqlinfo(
                    `Index of '${this.dataSchema.relatedTablesPrefix + tableSchema.tableName}' has created successfully`
                );
            }

        mconsole.sqlinfo(
            `Creating foreign key for related table ${tableSchema.tableName}' of schema '${this.dataSchema.tableName}'`
        );
        ...
        await this.sqlConnection.query(sql);
        mconsole.sqlinfo(
            `Foreign key for related table ${tableSchema.tableName}' of schema '${this.dataSchema.tableName}' has created successfully`
        );
    }
    ...
    protected async loadFromDB(): Promise<DataType> {
        ...
        if (this.id === undefined) {
            if (this._byUniqField === undefined)
                throw new DocumentError(
                    DocumentErrorCode.parameter_expected,
                    `Unique value of object '${this.constructor.name}' is undefined and id is undefined too`
                );
            ...
            [rows, fields] = await this.sqlConnection.query<[]>(sql, [
                this._byUniqField.value,
            ]);
            if (rows.length === 1) this._id = rows[0].id;
            else
                throw new DocumentError(
                    DocumentErrorCode.sql_not_found,
                    `There're ${rows.length} of records in '${this.dataSchema.tableName}'. Searched value '${this._byUniqField.value}' by field '${this._byUniqField.field}' Expected: 1`
                );
            mconsole.sqld(
                `Found only id = '${this._id}' in ${this.dataSchema.tableName} by field '${this._byUniqField.field}' = '${this._byUniqField.value}' `
            );
        }
        while (true) {
            try {
                [rows, fields] = await this.sqlConnection.query<[]>(
                    `select * from \`${this.dataSchema.tableName}\` where \`${this.dataSchema.idFieldName}\` = ?`,
                    [this.id]
                );
                break;
            } catch (e: any) {
        ...
        if (rows.length === 1) {
                    ...
                    while (true) {
                        try {
                            ...
                            >(
                                ...
                                where \`${this.dataSchema.relatedTablesPrefix + relObj.idFieldName}\` = ?`,
                                [this.id]
                            );
            ...
            throw new DocumentError(
                DocumentErrorCode.sql_not_found,
                `Object '${this.constructor.name}' with id = '${this.id}' not found`
            );
        }
    }
    ...
    async wfNext(
        predict: (availableStatuses: WorkflowStatusCode[]) => WorkflowStatusCode
    ): Promise<WorkflowStatusCode>;
    async wfNext(...arg: any[]): Promise<WorkflowStatusCode> {
        const availableTransfers = this.wfSchema.transfers?.filter(
            (transfer) => transfer.from === this.data.wfStatus
        );
