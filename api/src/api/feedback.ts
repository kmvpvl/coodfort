...
export async function updateFeedback(c: Context, req: Request, res: Response, user: User) {
    try {
        ...
        await prevFeedback.getCollection(
            `\`userId\`=? and \`objectId\`=? and \`objectType\`=? and \`answerToId\`${req.body.answerToId === undefined ? 'is null' : '=?'}`,
            [user.id, req.body.objectId, req.body.objectType, req.body.answerToId],
            '`created` DESC'
        );
...
export async function feedbackList(c: Context, req: Request, res: Response, user: User) {
    try {
        const prevFeedback = new Feedback();
        await prevFeedback.getCollection(`\`objectId\`=? and \`objectType\`=? ${req.body.userId !== undefined ? `and \`userId\`=?` : ''}`, [req.body.objectId, req.body.objectType, user.id], '`created` DESC');
